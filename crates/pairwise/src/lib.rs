...
<div class="warning"> We will probably relocate this </div>
...
* We represent each "piece of data" as an instance of the
  [`pairwise_nostd_internal::Datum`] type.
* We generally say that we produce  [`pairwise_nostd_internal::Datum`]
...
* We're only interested in calculations that can be implemented
  ...
  * as the algorithm considers each [`pairwise_nostd_internal::Datum`]
...
* we encode the logic for using a [`pairwise_nostd_internal::Datum`] to update
...
from each [`pairwise_nostd_internal::Datum`] instance that is produced from
a __data source__.
...
premise is that each [`pairwise_nostd_internal::Datum`] instance taken from a
...
code, we represent this pair as a [`pairwise_nostd_internal::BinnedDatum`]
...
[`pairwise_nostd_internal::TeamProps`] trait.
